...
class QcfFontLoader {
  ...
  static Future<void> _loadFontInternal(int pageNumber) async {
    ...
    try {
      ...
      // If we fail to load the font, we cannot proceed with registering it
      return;
    }
  ...
  /// Set the base URL for downloading assets
  static void setAssetBaseUrl(String url) {
    if (!url.endsWith('/')) {
      url += '/';
    }
    assetBaseUrl = url;
  ...
  /// Loads zipped font from remote server or assets
  static Future<Uint8List> _loadZip(String fontName) async {
    if (assetBaseUrl != null) {
      final url = '${assetBaseUrl}qcf_tajweed/$fontName.zip';
      try {
        final response = await http.get(Uri.parse(url));
        if (response.statusCode == 200) {
          return response.bodyBytes;
        } else {
          throw Exception("Failed to download font $fontName from $url (status: ${response.statusCode})");
        }
      } catch (e) {
        throw Exception("Error downloading font $fontName: $e");
      }
    } else {
